...
public class TimeShareCalculateYearDeviceRunningTimeTask {
    ...
    public void execute(String jobParam) {
        XxlJobHelper.log("######设备年分时运行时长统计【开始】######");
        log.info("######设备年分时运行时长统计【开始】######");
        ...
        String start = format.format(ofYearFirstLast.getOfDateFirst());
        String end = format.format(DateUtils.calculateMonth(ofYearFirstLast.getOfDateLast(), -1));
        ...
        for (Map.Entry<String, List<EmsMeter>> mapEntry : meterCompanyMap.entrySet()) {
            ...
            if (SysYesNoEnum._1.getCode().equals(jobSwitch)) {
                execute(busiKey, start, end, companyCode, meterList);
            } else {
        ...
        stopWatch.stop();
        XxlJobHelper.log("######设备年分时运行时长统计【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
        log.info("######设备年分时运行时长统计【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
    }

    private void execute(String busiKey, String start, String end, String companyCode, List<EmsMeter> meterList) {
        for (EmsMeter emsMeter : meterList) {
            ...
            EmsTimeShareDeviceRuntimeStatistics where = new EmsTimeShareDeviceRuntimeStatistics();
            where.setDeviceId(meterCode);
            where.setQryStartTime(start);
            where.setQryEndTime(end);
            where.setDataType(TemporalGranularityEnum.VD_Month.getCode());
            List<EmsTimeShareDeviceRuntimeStatistics> etsdList = emsTimeShareDeviceRuntimeStatisticsService.findList(where);
