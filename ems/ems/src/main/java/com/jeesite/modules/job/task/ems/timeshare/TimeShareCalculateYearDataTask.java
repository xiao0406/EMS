...
public class TimeShareCalculateYearDataTask {
    ...
    public void execute(String jobParam) {
        XxlJobHelper.log("######电表年分时电耗计算定时任务【开始】######");
        log.info("######电表年分时电耗计算定时任务【开始】######");
        ...
        Date ofYearEnd = DateUtils.calculateMonth(ofYearFirstLast.getOfDateLast(), -1);
        ...
        for (Map.Entry<String, List<EmsMeter>> mapEntry : meterCompanyMap.entrySet()) {
            ...
            if (SysYesNoEnum._1.getCode().equals(jobSwitch)) {
                execute(busiKey, ofYearFirst, ofYearEnd, companyCode, meterList);
            } else {
        ...
        stopWatch.stop();
        XxlJobHelper.log("######电表年分时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
        log.info("######电表年分时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
    }

    private void execute(String busiKey, Date ofYearFirst, Date ofYearEnd, String companyCode, List<EmsMeter> meterList) {
        //按设备去处理数据
        for (EmsMeter emsMeter : meterList) {
            ...
            EmsTimeSharePowerConsumptionStatistics where = new EmsTimeSharePowerConsumptionStatistics();
            where.setDeviceId(meterCode);
            where.setQryStartTime(DateUtils.formatDate(ofYearFirst, "yyyy-MM"));
            where.setQryEndTime(DateUtils.formatDate(ofYearEnd, "yyyy-MM"));
            where.setDataType(TemporalGranularityEnum.VD_Month.getCode());
            List<EmsTimeSharePowerConsumptionStatistics> tspcList = tmcStatisticsService.findList(where);
