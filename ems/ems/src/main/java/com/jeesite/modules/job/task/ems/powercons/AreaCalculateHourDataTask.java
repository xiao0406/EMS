...
public class AreaCalculateHourDataTask {
    ...
    public String getKEY_etlSchedule(String appointedTime) {
        ...
        String KEY_etlSchedule = RedisConstant._ETL_AREA_SCHEDULE_HOUR_ + busiKey;
    ...
    public Long execute(String jobParam, boolean totalGene) throws ParseException {
        XxlJobHelper.log("######区域1小时电耗计算定时任务【开始】######");
        log.info("######区域1小时电耗计算定时任务【开始】######");
        ...
        stopWatch.stop();
        XxlJobHelper.log("######区域1小时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
        log.info("######区域1小时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
    ...
    private void execute(Date optDTime, String companyCode, List<EmsArea> emsAreas) {
        for (EmsArea emsArea : emsAreas) {
            ...
            where.setDataType(TemporalGranularityEnum.VD_Hour.getCode());
...
//            //从数据库查询看当前计算执行的节点
//            List<Date> stockPendulum = emsElectricPowerAreaConsumptionService.getStockPendulum(busiKey, TemporalGranularityEnum.VD_Hour);
