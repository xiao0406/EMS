...
public class DeviceCalculateHourDataTask {
    ...
    private static final Integer CLOCK_STEP = 1;
    ...
    public String getKEY_etlSchedule(String appointedTime) {
        ...
        String KEY_etlSchedule = RedisConstant._ETL_SCHEDULE_HOUR_ + busiKey;
    ...
    public Long execute(String jobParam, boolean totalGene) throws ParseException {
        XxlJobHelper.log("######电表1小时电耗计算定时任务【开始】######");
        log.info("######电表1小时电耗计算定时任务【开始】######");
        ...
        if (optDTime.getTime() >= currentTimeMillis) {
            if (totalGene || optDTime.getTime() - currentTimeMillis >= Calculatehelper.ONE_HOUR_TIME_MILLIS) {
                XxlJobHelper.log("时间还没到，定时任务处理终止(做时间对比的时候要加上偏移量)");
                log.info("时间还没到，定时任务处理终止(做时间对比的时候要加上偏移量)");
                return Calculatehelper.ABORT_BIT_POS;
            }
        } else {
            XxlJobHelper.log("当前时间（{}）大于时刻表时间，改变时刻（{}）触发标志，当前时刻不再计算，进入下一个时刻", DateUtils.formatDate(new Date(currentTimeMillis), "yyyy-MM-dd HH:mm:ss"), DateUtils.formatDate(optDTime, "yyyy-MM-dd HH:mm:ss"));
            log.info("当前时间（{}）大于时刻表时间，改变时刻（{}）触发标志，当前时刻不再计算，进入下一个时刻", DateUtils.formatDate(new Date(currentTimeMillis), "yyyy-MM-dd HH:mm:ss"), DateUtils.formatDate(optDTime, "yyyy-MM-dd HH:mm:ss"));
            optFlag = true;
        }
        ...
        //当前将要计算时刻  上一个时刻
        Date lastOptDTime = DateUtils.calculateHour(optDTime, -1 * CLOCK_STEP);
        ...
        stopWatch.stop();
        XxlJobHelper.log("######电表1小时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
        log.info("######电表1小时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
    ...
    private void execute(Date optDTime, Date lastOptDTime, String companyCode, List<EmsMeter> meterList) {
        ...
        for (EmsMeter emsMeter : meterList) {
            ...
            builder
                    ...
                    .dataType(TemporalGranularityEnum.VD_Hour.getCode())        // 数据类型：15分钟、小时、日、月、年
    ...
    private void refreshCacheIfNotExist(String busiKey, String KEY_etlSchedule) {
        ...
        if (!isExist) {
            ...
            List<String> pendulum = new Clock().pendulum(TimeUnitEnum.YMD_04, CLOCK_STEP);
            //此处一定要保证有序
...
//            //从数据库查询看当前计算执行的节点
//            List<Date> stockPendulum = powerConsumptionService.getStockPendulum(busiKey, TemporalGranularityEnum.VD_Hour);
