...
public class TimeShareCalculateAreaMonthDataTask {
    ...
    public void execute(String jobParam) {
        XxlJobHelper.log("######区域每月分时电耗计算定时任务【开始】######");
        log.info("######区域每月分时电耗计算定时任务【开始】######");
        ...
        stopWatch.stop();
        XxlJobHelper.log("######区域每月分时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
        log.info("######区域每月分时电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
    }
    ...
    private void execute(String busiKey, String companyCode, List<EmsArea> emsAreas) {
        for (EmsArea emsArea : emsAreas) {
            ...
            EmsTimeShareAreaPowerConsumptionStatistics areaConsumption = emsTimeSharePowerConsumptionStatisticsService.getAreaStageCumulativeConsumption(where);
            ...
            //查询存量数据
            EmsTimeShareAreaPowerConsumptionStatistics stockedRec = emsTimeShareAreaPowerConsumptionStatisticsService.isStockedRec(areaConsumption);
            ...
            emsTimeShareAreaPowerConsumptionStatisticsService.save(areaConsumption);
