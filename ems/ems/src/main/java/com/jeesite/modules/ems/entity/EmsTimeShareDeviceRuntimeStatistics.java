 ...
 * @version 2023-06-27
 */
@Table(name="ems_time_share_device_runtime_statistics", alias="a", label="峰平谷设备运行时长统计表信息", columns={
...
@AllArgsConstructor
@Data
@ApiModel(value = "EmsTimeShareDeviceRuntimeStatistics对象", description = "峰平谷设备运行时长统计表Entity")
public class EmsTimeShareDeviceRuntimeStatistics extends DataEntity<EmsTimeShareDeviceRuntimeStatistics> {
	...
	@ExcelFields({
			@ExcelField(title="时间", attrName = "dataDateKey", align = ExcelField.Align.CENTER, sort = 10, dataFormat="yyyy-MM-dd"),
			...
			@ExcelField(title="停机时长（小时）", attrName="totalStop", align= ExcelField.Align.CENTER, sort=40),
			@ExcelField(title="空载时长（小时）", attrName="totalNoLoad", align= ExcelField.Align.CENTER, sort=40),
			@ExcelField(title="运行时长（小时）", attrName="totalRunning", align= ExcelField.Align.CENTER, sort=40),
	})
	public EmsTimeShareDeviceRuntimeStatistics() {
		this(null);
	}
	
	public EmsTimeShareDeviceRuntimeStatistics(String id){
		super(id);
	}
