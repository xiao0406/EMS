...
public class DeviceCalculateMonthDataTask {
    ...
    public void execute(String jobParam) throws ParseException {
        XxlJobHelper.log("######电表每月电耗计算定时任务【开始】######");
        log.info("######电表每月电耗计算定时任务【开始】######");
        ...
        //当前将要计算时刻  上一个时刻
        Date timeStart = ofMonthFirstLast.getOfDateFirst();
        Date timeEnd = DateUtils.calculateDay(ofMonthFirstLast.getOfDateLast(), -1);
        ...
        stopWatch.stop();
        XxlJobHelper.log("######电表每月电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
        log.info("######电表每月电耗计算定时任务【结束】######，耗时=》【{}s】", stopWatch.getTotalTimeSeconds());
    }
    ...
    private void execute(String busiKey, Date timeStart, Date timeEnd, String companyCode, List<EmsMeter> meterList) {
        ...
        params.setQryStartTime(timeStart);
        params.setQryEndTime(timeEnd);
        ...
        for (EmsMeter emsMeter : meterList) {
            ...
            EmsElectricPowerConsumption lastData = null;
